...
(provide head menu principal-produto other-list process-all-tamanho)

(define (ctags name intro [clss ""])
  (format "<~a~a>~a</~a>\n\t" name clss intro name)) 

(define (process-all-tamanho sx)
  (let tamanho((xl (regexp-match* #rx"([0-9]|[GPM])+" sx)) (rst ""))
    (if (empty? xl) rst
        (tamanho (cdr xl) (string-append rst (ctags "option" (car xl)))) )))
...
(define (principal-produto xl [links "ver.rkt"] [c 6] [i 1])
 (let ((nome (first xl)) (marca (second xl)) (valor (third xl)) (image (fourth xl)) (xid (fifth xl)))
  (string-append "" (ctags "a" (~a (ctags "p" (~a "$" valor #:max-width 5) " id=\"preco-principal\"")
                                   ...
                                   (ctags "p" (~a (ctags "span" marca " class=\"font-weight-bold d-block\"")
                                                  nome)
                                          " id=\"nome-principal\""))
                           (~a " href=\"" (~a links "?" xid "\"") " class=\"produto-principal col-lg-" c " bg-" i "\"") ))))



(define (other-list xl [links "ver.rkt"] [c 6] [index 2])
  (let build((rst "") (n (first xl)) (m (second xl)) (v (third xl)) (i (fourth xl)) (xid (fifth xl)) (k index))
    (if (or (empty? n) (empty? m) (empty? v) (empty? i))
        ...
        (build (string-append rst (ctags "a" (~a (ctags "p" (~a "$" (car v) #:max-width 5) " class=\"preco-produto\"")
                                                 ...
                                                 (ctags "p" (~a (ctags "span" (car m) " class=\"font-weight-bold d-bock\"")
                                                                (car n))
                                                        " class=\"nome-produto\""))
                                         (~a " href=\"" (~a links "?" (car xid) "\"") " class=\"produto col-lg-" c " bg-" k "\"")))
               (cdr n) (cdr m) (cdr v) (cdr i) (cdr xid) (add1 k)) )))
